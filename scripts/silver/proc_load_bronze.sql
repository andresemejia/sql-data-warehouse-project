...
BEGIN
	...
	SELECT
		prd_id,
		REPLACE(SUBSTRING(prd_key, 1, 5), '-', '_') AS cat_id, --- Split prd_key to show cat_id (CategoryID)
		...
		CAST(LEAD(prd_start_dt) OVER (PARTITION BY prd_key ORDER BY prd_start_dt) -1 AS DATE) AS prd_end_date -- Calculate end date before the next start date
	...
	CASE WHEN sls_sales IS NULL OR sls_sales <=0 OR sls_sales != sls_quantity * ABS(sls_price)
		...
		ELSE sls_sales
	END AS sls_sls, -- Recalculate sales if original value is missing or incorrect
	sls_quantity,
